...
async def check_premium_access_by_phone(phone: str) -> tuple:
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with # File: main.py — бот Salesplan для MAX (бесплатная диагностика + Premium)
...
def get_challenge_progress(user_id: str) -> dict:
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.execute("""
        SELECT current_day, tasks_completed FROM challenges 
        WHERE user_id = ? AND status = 'active'
        ORDER BY start_date DESC LIMIT 1
    """, (user_id,))
    row = cursor.fetchone()
    conn.close()
    if row:
        return {"current_day": row[0], "tasks_completed": row[1]}
    return None
