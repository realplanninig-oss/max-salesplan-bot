# File: main.py — упрощенная версия для теста
...
# === ДИАГНОСТИКА ===
print("=" * 60)
print("ENVIRONMENT VARIABLES CHECK - MAX Bot (SIMPLE TEST)")
print("=" * 60)
...
async def lifespan(app: FastAPI):
    logger.info("Bot started (simple test version)")
    yield
    logger.info("Bot stopped")

app = FastAPI(title="MAX Bot Simple Test", lifespan=lifespan)
...
@app.get("/")
async def root():
    return {"status": "MAX Bot is running", "version": "test"}
    ...
    try:
        ...
        if "message" in payload:
            ...
            if user_id:
                ...
                if text == "/start":
                    result = await send_message(str(user_id), 
                        "Привет! Я тестовый бот. Если ты видишь это сообщение, базовая отправка работает!")
                    logger.info(f"Send result: {result}")
                else:
                    result = await send_message(str(user_id), f"Ты написал: {text}")
                    logger.info(f"Send result: {result}")
